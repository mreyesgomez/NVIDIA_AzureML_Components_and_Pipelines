...
while(True):
    ...
    for file in container_client.walk_blobs('inputs/', delimiter='/'):
        ...
        if ext == '.mp4' or ext == '.mov':
            ...
            os.chdir('/tmp')
            os.system('./deepstream-app config.txt')
            os.system('mv out.mp4 '+ filename_no_ext + '.mp4')
            ...
            try:
                with open('/tmp/' + filename_no_ext + '.mp4',"rb") as f:
                    blob.upload_blob(f)
                    print('out.mp4 uploaded to container: ' + args.container_name + ' successfully')
            except Exception as e:
                print(e)
